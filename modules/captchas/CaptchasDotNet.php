...
{
  function CaptchasDotNet ($client, $secret, $random_repository = 
  ...
  function url ($random = False, $base = 'http://image.captchas.net/')
  {
    if (!$random)
    {
    ...
    return $base . '?client=' . $this->__client . '&amp;random=' . $random;
  }
...
EOT;
    ...
    $image = str_replace ('@URL@', $this->url (), $image);
    return $image;
  }
  ...
  function verify ($input, $random = False)
  {
    ...
    $digest = md5 ($this->__secret . $random);
